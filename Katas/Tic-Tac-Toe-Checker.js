...
const isSolved = board => {
  ...
  let winner = -1;
  board.forEach(row => {
    if (row.every(v => v === row[0]) && row[0] != 0) winner = row[0];
  });
  ...
  //Check for draw
  if (board.every(x => x.every(y => y != 0))) winner = 0;
...
function isSolved(board) {
  ...
  return /222|2...2...2|2....2....2|2..2..2/.test(board)
    ? 2
    : /111|1...1...1|1....1....1|1..1..1/.test(board)
    ? 1
    : /0/.test(board)
    ? -1
    : 0;
